...
class TrajectorySampler(ABC):
    ...
    def sample(self, initial_state_goals: list[tuple[mm.State, mm.GroundConjunctiveCondition]], model: QValueModel, reward_function: RewardFunction, horizon: int) -> list[Trajectory]:
        ...
        with torch.no_grad():
            ...
            for _ in range(horizon):
                ...
                for rollout_idx, (q_values, applicable_actions) in zip(context_indices, q_values_batch):
                    ...
                    successor_state = action.apply(current_state)
                    reward = reward_function(current_state, action, successor_state, context.goal_condition)
        ...
        for context in rollout_contexts:
            trajectories.append(Trajectory(context.state_sequence, context.action_sequence, context.reward_sequence, context.goal_condition))
        return trajectories
